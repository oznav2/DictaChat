...
def name_needs_alias(name: str) -> bool:
    """Check if a field name needs aliasing (if it starts with '_' or conflicts with BaseModel attributes)."""
    # Pydantic BaseModel reserved fields include 'schema', 'model_dump', etc.
    # We should alias 'schema' to avoid conflicts.
    return name.startswith("_") or name == "schema"
...
def generate_alias_name(original_name: str, existing_names: set) -> str:
    ...
    Args:
        original_name: The original field name
