...
css = """
/* RTL and Bidi support for Hebrew */
[dir="rtl"] {
    direction: rtl;
    unicode-bidi: embed;
}
...
/* Input and button styling for RTL */
...
/* Markdown rendered content - support both */
.gradio-markdown {
    direction: rtl;
    unicode-bidi: embed;
    text-align: right;
...
/* Override Svelte-generated markdown direction */
.gradio-markdown [data-testid="markdown"] {
    direction: rtl !important;
    unicode-bidi: embed !important;
    text-align: right !important;
}

/* Target all nested elements in rendered markdown */
.gradio-markdown .prose,
.gradio-markdown .prose *,
.gradio-markdown .svelte-vuh1yp,
.gradio-markdown .svelte-lag733,
.gradio-markdown .svelte-7ddecg {
    direction: rtl !important;
    unicode-bidi: embed !important;
    text-align: right !important;
}

/* Keep code blocks LTR within RTL context */
.gradio-markdown code,
.gradio-markdown pre,
.gradio-markdown .prose code,
.gradio-markdown .prose pre {
    direction: ltr !important;
    unicode-bidi: embed !important;
    text-align: left !important;
}

/* Force RTL for paragraphs and text elements */
.gradio-markdown p,
.gradio-markdown span,
.gradio-markdown div:not(.prose):not([data-testid="markdown"]) {
    direction: rtl !important;
    unicode-bidi: embed !important;
    text-align: right !important;
}

.gradio-markdown code, .gradio-markdown pre {
    direction: ltr;
    unicode-bidi: embed;
    text-align: left;
}

/* Specific targeting for our RTL markdown component */
#markdown-rtl,
.markdown-rtl-content,
#markdown-rtl [data-testid="markdown"],
.markdown-rtl-content [data-testid="markdown"] {
    direction: rtl !important;
    unicode-bidi: embed !important;
    text-align: right !important;
}
...
#markdown-rtl .prose,
#markdown-rtl .prose *,
.markdown-rtl-content .prose,
.markdown-rtl-content .prose *,
#markdown-rtl .svelte-vuh1yp,
#markdown-rtl .svelte-lag733,
#markdown-rtl .svelte-7ddecg {
    direction: rtl !important;
    unicode-bidi: embed !important;
    text-align: right !important;
}
...
/* RTL for all tabs content */
#markdown-render-tab, #docling-render-tab, #markdown-tab, #docling-json-tab, #html-tab, #html-rendered-tab, #text-tab, #doctags-tab {
    direction: rtl !important;
    unicode-bidi: embed !important;
}

/* Specific content adjustments within tabs */
#markdown-tab .gradio-code,
#docling-json-tab .gradio-code,
#html-tab .gradio-code,
#text-tab .gradio-code,
#doctags-tab .gradio-code {
    direction: ltr !important;
    text-align: left !important;
}

/* Ensure HTML rendered content is RTL */
#html-rendered-tab .gradio-html {
    direction: rtl !important;
    text-align: right !important;
}
...
/* Row layout adjustments for RTL */
.gradio-row {
...
def process_file(
    ...
    files_data = []
    for file in files:
        # Hack: Rename .txt to .md so Docling accepts it as Markdown
        filename = file.name
        ...
        files_data.append({
            "kind": "file", 
            "base64_string": file_to_base64(file), 
            "filename": filename
        })
...
with gr.Blocks(
    ...
    css=css,
    theme=theme,
    title="DictaChat Document Extraction Engine",
    ...
    processing_text = gr.State("מעבד את המסמך שביקשת..אנא המתן...")
    ...
    with gr.Row(elem_id="check_health"):
        ...
        with gr.Column(scale=16, elem_id="dark_mode_column"):
            dark_mode_btn = gr.Button("שינוי מצב בהיר/כהה", scale=0)
            dark_mode_btn.click(
                None,
                None,
                None,
    ...
    # URL Processing Tab
    with gr.Tab("המרת טקסט מלינק"):
        with gr.Row():
            with gr.Column(scale=4):
                url_input = gr.Textbox(
                    label="כתובת URL",
                    placeholder="למשל העתיקו כאן: https://arxiv.org/pdf/2501.17887",
                )
            with gr.Column(scale=1):
                url_process_btn = gr.Button("התחל עיבוד", scale=1)
                url_reset_btn = gr.Button("איפוס", scale=1)

    # File Processing Tab
    with gr.Tab("המרת קובץ"):
        with gr.Row():
            with gr.Column(scale=4):
                raw_exts = itertools.chain.from_iterable(FormatToExtensions.values())
                # Add .txt support manually by treating it as Markdown
                file_input = gr.File(
            ...
            with gr.Column(scale=1):
                file_process_btn = gr.Button("התחל עיבוד", scale=1)
                file_reset_btn = gr.Button("איפוס", scale=1)
    ...
    with gr.Row(visible=bool(docling_serve_settings.api_key)):
        with gr.Column():
            auth = gr.Textbox(
                label="טוקן אימות",
                placeholder="הכנס את הטוקן האימות כאן",
                type="password",
            )

    # Options
    with gr.Accordion("אפשרויות") as options:
        with gr.Row():
            with gr.Column(scale=1):
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1, min_width=200):
                ocr_lang = gr.Textbox(
                    label="שפת OCR (מזהה פורמט)",
                    value="en,he,fr,de,es",
                    visible=False,
                )
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                abort_on_error = gr.Checkbox(label="עצירה כשמתרחשת שגיאה", value=False)
                return_as_file = gr.Checkbox(label="החזרת קובץ", value=False)
        with gr.Row():
            with gr.Column():
                do_code_enrichment = gr.Checkbox(
                    label="שיפור תצוגת קוד", value=False
                )
                do_formula_enrichment = gr.Checkbox(
                    label="שיפור תצוגת נוסחאות", value=False
                )
            with gr.Column():
                do_picture_classification = gr.Checkbox(
                    label="אפשר קיטלוג תמונות", value=False
                )
                do_picture_description = gr.Checkbox(
                    label="תיאור תמונה", value=False
                )
    ...
    with gr.Row(visible=False) as content_output:
        ...
        with gr.Tab("מארקדאון מסוגנן"):
            output_markdown_rendered = gr.Markdown(
                label="Response",
                elem_id="markdown-rtl",
                elem_classes=["markdown-rtl-content"]
            )
        with gr.Tab("HTML"):
