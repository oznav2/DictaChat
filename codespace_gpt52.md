# Strategic Enhancement Blueprint (GPT-5.2)
...
- “Existing but unused” functions in `src/lib/server/textGeneration/mcp/memoryIntegration.ts`:
  ...
  - The meaningful missing wiring is **enforced tool gating** (filter tools list pre-inference / pre-tool-execution) and **tool result ingestion** (non-docling).
...
**Core problem**
- First message lacks user profile context, causing low personalization.
...
**Core problem**
- Model must indicate which memories helped so learning can be targeted.
