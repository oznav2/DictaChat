	...
	describe('TTL Expiration', () => {
		it('test_ttl_expiration_detection', async () => {
			...
			try {
				...
				const checkExpired = (docs: Array<{ document: { id: string; metadata: Record<string, unknown> } }>, checkTime: number): number => {
	...
	describe('Automatic Staleness Flagging', () => {
		it('test_auto_staleness_detection', async () => {
			...
			try {
				...
				const contentTypes = [
					{ id: 'weather', content: 'Weather in Tel Aviv is sunny', ttl: TTL_SHORT, type: 'ephemeral' },
					{ id: 'schedule', content: 'User has meeting at 3pm today', ttl: TTL_SHORT, type: 'ephemeral' },
					{ id: 'preference', content: 'User prefers dark mode', ttl: TTL_MEDIUM, type: 'preference' },
	...
	describe('Update vs Replace Decisions', () => {
		it('test_update_vs_replace', async () => {
			...
			try {
				...
				const replacements = results.filter(r => r.document.metadata.update_type === 'replacement').length;
				const additions = results.filter(r => r.document.metadata.update_type === 'addition').length;
		...
		it('test_hebrew_tombstone', async () => {
			...
			try {
				...
				metrics = {
					...
					hebrew_tombstones: results.filter(r => r.document.metadata.status === 'tombstone').length,
				};
		...
		it('test_access_frequency_decay', async () => {
			...
			try {
				...
				metrics = {
					...
					high_access_count: results.filter(r => (r.document.metadata.access_count as number) > 10).length,
					low_access_count: results.filter(r => (r.document.metadata.access_count as number) <= 2).length,
				};
		...
		it('test_stale_data', async () => {
			...
			try {
				...
				const testData = [
					...
					// Aging data
					{ id: 's3', content: 'User mentioned coffee preference', age: TTL_MEDIUM * 0.5, ttl: TTL_MEDIUM },
					...
					// Tombstoned
					{ id: 's7', content: '[DELETED] Old preference', age: TTL_LONG, ttl: TTL_MEDIUM, status: 'tombstone' },
					{ id: 's8', content: '[נמחק] העדפה ישנה', age: TTL_LONG, ttl: TTL_MEDIUM, status: 'tombstone' },
				];
				...
				const stale = results.filter(r => (r.document.metadata.freshness_score as number) <= 0.5 && r.document.metadata.status !== 'tombstone');
				const tombstones = results.filter(r => r.document.metadata.status === 'tombstone');

				// Check language distribution
				const hebrewItems = results.filter(r =>
					(r.document.content as string).match(/[\u0590-\u05FF]/) !== null
				);
