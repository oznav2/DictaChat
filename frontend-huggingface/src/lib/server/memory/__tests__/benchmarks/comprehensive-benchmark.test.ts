...
function generateHebrewContent(): Array<{
	...
	return [
		...
		{
			id: "he_3",
			content: "ים המלח הוא הנקודה הנמוכה ביותר בעולם",
			metadata: { language: "hebrew" },
		},
...
function generateReport(results: BenchmarkResult[], summaries: ConditionSummary[]): string {
	const lines: string[] = [
		"=".repeat(100),
	...
	for (const condition of CONDITIONS) {
		...
		for (const result of conditionResults) {
			...
			lines.push(
				`    MRR: ${result.mrr.toFixed(4)} | nDCG: ${result.ndcg.toFixed(4)} | P@5: ${result.precision.toFixed(4)}`
			);
...
describe("Comprehensive Benchmark", () => {
		...
		for (const condition of CONDITIONS) {
			...
			if (conditionResults.length > 0) {
				conditionSummaries.push({
					...
					avg_precision:
						conditionResults.reduce((a, r) => a + r.precision, 0) / conditionResults.length,
					avg_latency_ms:
						conditionResults.reduce((a, r) => a + r.latency_ms, 0) / conditionResults.length,
					pass_rate:
						conditionResults.filter((r) => r.passes_quality).length / conditionResults.length,
				});
			}
		}
	...
	describe("Cold Start Condition", () => {
		for (const maturity of MATURITY_NAMES) {
			it(`should perform well at ${maturity} maturity level`, async () => {
				...
				const relevantIds = new Set(Array.from({ length: 10 }, (_, i) => `technology_${i}`));

				const metrics = calculateAllMetrics(resultIds, relevantIds, 5);
				const latency = performance.now() - startTime;

				const passes = metrics.mrr >= QUALITY_TARGETS.mrr && metrics.ndcg >= QUALITY_TARGETS.ndcg;
	...
	describe("With Context Condition", () => {
		for (const maturity of MATURITY_NAMES) {
			it(`should perform well at ${maturity} maturity level with context`, async () => {
				...
				const passes =
					metrics.mrr >= QUALITY_TARGETS.mrr * 0.8 && // Slightly relaxed
	...
	describe("Cross Domain Condition", () => {
		for (const maturity of MATURITY_NAMES) {
			it(`should generalize to unseen domains at ${maturity} maturity`, async () => {
				...
				const relevantIds = new Set(Array.from({ length: 10 }, (_, i) => `medicine_${i}`));
				...
				const passes =
					metrics.mrr >= QUALITY_TARGETS.mrr * 0.6 && metrics.ndcg >= QUALITY_TARGETS.ndcg * 0.6;
	...
	describe("Maturity Progression", () => {
		it("should show improved performance as maturity increases", async () => {
			...
			const isMonotonic = scores.every((val, i) => i === 0 || val >= scores[i - 1]);
