...
function generateReport(results: TortureResult[]): string {
	...
	lines.push(
		`  Overall: ${passCount === results.length ? "ALL TESTS PASSED" : "SOME TESTS FAILED"}`
	);
	lines.push("=".repeat(100));
...
describe("Torture Test Suite", () => {
	...
	describe("3. Concurrent Operations", () => {
		it("should handle concurrent insertions", async () => {
			...
			const promises = Array.from({ length: STRESS_CONFIG.concurrent_operations }, (_, i) =>
				collection
					.add({
		...
		it("should handle concurrent searches", async () => {
			...
			const promises = Array.from({ length: STRESS_CONFIG.concurrent_operations }, (_, i) =>
				collection.search(`concurrent query ${i}`, 5).catch(() => {
					errors++;
				})
			);
		...
		it("should handle mixed concurrent operations", async () => {
			...
			const mixedOps = Array.from({ length: 30 }, (_, i) => {
				if (i % 3 === 0) {
					...
					return collection.search(`mixed query ${i}`, 5).catch(() => {
						errors++;
					});
				} else {
					return embeddingService.embed(`mixed embed ${i}`).catch(() => {
						errors++;
					});
				}
			});
	...
	describe("6. Unicode and RTL Handling", () => {
		...
		it("should search with Hebrew queries", async () => {
			...
			const hebrewQueries = ["מה זה TypeScript?", "איך עובד React?", "שלום עולם"];
