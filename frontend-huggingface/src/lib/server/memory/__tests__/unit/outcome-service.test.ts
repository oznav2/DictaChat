...
describe('OutcomeServiceImpl', () => {
	...
	describe('Record Response', () => {
		it('should store key takeaway', async () => {
			...
			const result: TestResult = {
				...
				passed: storeCall.text === 'User prefers TypeScript over JavaScript' &&
		...
		it('should embed key takeaway', async () => {
			...
			const result: TestResult = {
				...
				passed: mockEmbedding.embed.mock.calls.length === 1 &&
		...
		it('should index takeaway in Qdrant', async () => {
			...
			const result: TestResult = {
				...
				passed: upsertCall.payload.content === 'Index this content' &&
		...
		it('should use default unknown outcome', async () => {
			...
			const result: TestResult = {
				...
				passed: storeCall.metadata.initial_outcome === 'unknown' &&
	...
	describe('Related Memory Resolution', () => {
		it('should resolve positional references', async () => {
			...
			const recordedIds = mockMongo.recordOutcome.mock.calls.map(
				(call: any[]) => call[0].memoryId
			);

			const result: TestResult = {
				name: 'resolve_positional_refs',
				passed: recordedIds.includes('mem_first') && recordedIds.includes('mem_third') &&
	...
	describe('Error Handling', () => {
		...
		it('should handle embedding failure gracefully', async () => {
			...
			const result: TestResult = {
				...
				passed: mockMongo.store.mock.calls.length === 0 && mockQdrant.upsert.mock.calls.length === 0,
				duration: Date.now() - startTime
			};
