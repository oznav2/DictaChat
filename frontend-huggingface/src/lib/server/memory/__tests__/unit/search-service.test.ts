...
describe('SearchServiceImpl', () => {
	...
	describe('Basic Search', () => {
		...
		it('should search specific tiers when specified', async () => {
			...
			const result: TestResult = {
				...
				passed: callArgs.tiers.length === 2 &&
		...
		it('should include debug information in response', async () => {
			...
			const mockHybridSearch: MockHybridSearchService = {
				search: vi.fn().mockResolvedValue({
					...
					debug: {
						confidence: 'high',
						stage_timings_ms: { vector_search: 50 },
						fallbacks_used: [],
						errors: []
					}
				})
			...
			const result: TestResult = {
				...
				passed: response.debug !== undefined &&
					response.debug.stage_timings_ms !== undefined &&
					response.debug.stage_timings_ms.total_ms !== undefined,
				duration: Date.now() - startTime
			};
	...
	describe('Position Tracking', () => {
		it('should track position map after search', async () => {
			...
			const mockResults: MockSearchResult[] = [
				{ memory_id: 'mem_1', content: 'First', position: 1, score_summary: { final_score: 0.9 }, citations: [] },
				{ memory_id: 'mem_2', content: 'Second', position: 2, score_summary: { final_score: 0.8 }, citations: [] },
				{ memory_id: 'mem_3', content: 'Third', position: 3, score_summary: { final_score: 0.7 }, citations: [] }
			];
		...
		it('should clear turn tracking', async () => {
			...
			const mockResults: MockSearchResult[] = [
				{ memory_id: 'mem_1', content: 'Content', position: 1, score_summary: { final_score: 0.9 }, citations: [] }
			];
			...
			const result: TestResult = {
				...
				passed: service.getSearchPositionMap().size === 0 &&
		...
		it('should update tracking on each search', async () => {
			...
			const mockHybridSearch: MockHybridSearchService = {
				search: vi.fn()
					.mockResolvedValueOnce({
						results: [{ memory_id: 'first_search', content: 'A', position: 1, score_summary: { final_score: 0.9 }, citations: [] }],
						debug: { confidence: 'high', stage_timings_ms: {}, fallbacks_used: [], errors: [] }
					})
					.mockResolvedValueOnce({
						results: [{ memory_id: 'second_search', content: 'B', position: 1, score_summary: { final_score: 0.8 }, citations: [] }],
						debug: { confidence: 'high', stage_timings_ms: {}, fallbacks_used: [], errors: [] }
					})
			};
			...
			const result: TestResult = {
				...
				passed: service.getLastQueryNormalized() === 'second query' &&
	...
	describe('Error Handling', () => {
		...
		it('should handle empty results gracefully', async () => {
			...
			const mockHybridSearch: MockHybridSearchService = {
				search: vi.fn().mockResolvedValue({
					results: [],
					debug: { confidence: 'low', stage_timings_ms: {}, fallbacks_used: ['no_results'], errors: [] }
				})
			};
