	...
	describe('Initialization', () => {
		...
		it('should create facade with custom config', async () => {
			...
			const result: TestResult = {
				...
				passed: config.contextLimit === 5000,
				duration: Date.now() - startTime
			};

			harness.recordResult(result);
			expect(config.contextLimit).toBe(5000);
		});
		...
		it('should allow setting custom instance', async () => {
			...
			const customFacade = new UnifiedMemoryFacade({
				config: { contextLimit: 9999 } as any
			});
			...
			const result: TestResult = {
				...
				passed: retrieved.getConfig().contextLimit === 9999,
				duration: Date.now() - startTime
			};

			harness.recordResult(result);
			expect(retrieved.getConfig().contextLimit).toBe(9999);
	...
	describe('Search Operations', () => {
		...
		it('should search with collection filter', async () => {
			...
			const result: TestResult = {
				...
				passed: callArgs.collections.includes('working') && callArgs.collections.includes('patterns'),
				duration: Date.now() - startTime
			};
	...
	describe('Outcome Recording', () => {
		it('should record positive outcome', async () => {
			...
			const result: TestResult = {
				...
				passed: callArgs.outcome === 'positive' && callArgs.relatedMemoryIds.length === 2,
				duration: Date.now() - startTime
			};
