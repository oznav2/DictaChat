...
import {
  ...
  createDocumentContextStore
} from "../stores/documentContextStore";
...
export class DocumentRAGService {
  ...
  async augmentContext(params: AugmentParams): Promise<void> {
    ...
    const newEmbeddings = await this.embedder.embedBatch(
  ...
  private extractKeywords(text: string): { hebrew: string[]; english: string[] } {
    ...
    const sortedHebrew = [...hebrewCounts.entries()]
      .sort((a, b) => b[1] - a[1])
      .slice(0, 10);
    ...
    const sortedEnglish = [...englishCounts.entries()]
      .sort((a, b) => b[1] - a[1])
      .slice(0, 10);
   ...
   */
  private extractUserContextSync(
    query: string,
    language: SupportedLanguage
  ): UserContext {
