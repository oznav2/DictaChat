...
export class DocumentContextStore {
  ...
  async createDocumentContext(
    data: Omit<
      DocumentContext,
      "_id" | "createdAt" | "lastAccessedAt" | "augmentationHistory"
    >
  ...
  async createChunks(
    chunks: Array<
      Omit<DocumentChunk, "_id" | "retrievalCount" | "averageRerankerScore">
    >
  ): Promise<void> {
   ...
   */
  async addLearnedFact(
    conversationId: string,
    fact: Omit<LearnedFact, "addedAt">
  ): Promise<void> {
