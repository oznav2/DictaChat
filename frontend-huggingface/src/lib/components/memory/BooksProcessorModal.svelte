<script lang="ts">
	...
	const ALLOWED_EXTENSIONS = [".txt", ".md", ".pdf", ".docx", ".xlsx", ".xls", ".csv", ".tsv", ".html", ".htm", ".rtf"];
	...
	async function processFile(file: ProcessingFile) {
		files = files.map((f) =>
			f.id === file.id ? { ...f, status: "uploading" as const, message: "מעלה קובץ...", progress: 5 } : f
		);
		...
		try {
			...
			if (!title) {
				files = files.map((f) =>
					f.id === file.id ? { ...f, status: "error" as const, error: "נדרש כותרת", message: "אנא הזן כותרת" } : f
				);
				return;
			}
			...
			if (!data.success && data.processing_status === "duplicate") {
				files = files.map((f) =>
					f.id === file.id
						? { ...f, status: "error" as const, error: data.message || "הספר כבר קיים בספריה", message: "נמצא כפילות", progress: 0 }
						: f
				);
				return;
			}
			...
			if (data.task_id) {
				files = files.map((f) =>
					f.id === file.id
						? { ...f, status: "processing" as const, taskId: data.task_id, bookId: data.book_id, message: "מעבד ספר...", progress: 10 }
						: f
				);

				// Poll for progress
				pollProgress(file.id, data.task_id, data.book_id);
			}
		} catch (err) {
			console.error("Error processing file:", err);
			files = files.map((f) =>
				f.id === file.id
					? { ...f, status: "error" as const, progress: 0, error: err instanceof Error ? err.message : "שגיאה לא ידועה", message: "העלאה נכשלה" }
					: f
			);
		}
	}
	...
	async function pollProgress(fileId: string, taskId: string, bookId: string) {
		...
		const poll = async () => {
			if (attempts >= maxAttempts) {
				files = files.map((f) =>
					f.id === fileId ? { ...f, status: "error" as const, error: "Timeout - העיבוד לקח יותר מדי זמן" } : f
				);
				return;
			}
			...
			try {
				...
				if (response.ok) {
					...
					if (book.status === "completed") {
						files = files.map((f) =>
							f.id === fileId ? { ...f, status: "completed" as const, progress: 100, message: "העיבוד הושלם!", totalChunks: book.processing_stats?.total_chunks } : f
						);
						...
						files = files.map((f) =>
							f.id === fileId ? { ...f, status: "error" as const, error: book.error || "העיבוד נכשל" } : f
						);
						return;
					} else {
						// Still processing
						const progress = book.processing_stats?.total_chunks
							? Math.round((book.processing_stats.chunks_processed / book.processing_stats.total_chunks) * 90) + 10
							: 10;
						files = files.map((f) =>
							f.id === fileId ? { ...f, progress, message: `מעבד... ${book.processing_stats?.chunks_processed || 0}/${book.processing_stats?.total_chunks || "?"}` } : f
						);
					}
				}
			} catch (err) {
	...
	async function deleteBook() {
		...
		try {
			const response = await fetch(`${base}/api/memory/books/${deleteConfirm.bookId}`, {
				method: "DELETE",
			});

			if (response.ok) {
				existingBooks = existingBooks.filter((b) => b.book_id !== deleteConfirm.bookId);
				deleteConfirm = null;
			} else {
...
{#if isOpen}
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" dir="rtl">
		<div class="flex max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-xl border border-gray-700 bg-gray-900 shadow-2xl">
			<!-- Header -->
			<div class="flex items-center justify-between border-b border-gray-700 p-6">
				...
				<button onclick={handleClose} class="rounded-lg p-2 transition-colors hover:bg-gray-800" aria-label="סגור">
			...
			<div class="border-b border-gray-700 px-6">
				<div class="flex gap-4">
					<button
						...
						class={["border-b-2 px-1 py-3 transition-colors", !showExisting ? "border-blue-400 text-gray-100" : "border-transparent text-gray-400 hover:text-gray-200"]}
					>
						העלאה חדשה
					</button>
					<button
			...
			<div class="flex-1 overflow-y-auto p-6">
				{#if !showExisting}
					...
					<div
						...
						class={["rounded-lg border-2 border-dashed p-8 text-center transition-all", isDragging ? "border-blue-400 bg-blue-400/10" : "border-gray-600 bg-gray-800/50 hover:border-gray-500"]}
					>
						<svg class="mx-auto mb-4 size-12 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
						</svg>
						<p class="mb-2 text-gray-300">
							גרור ושחרר את המסמכים שלך כאן, או
							<button onclick={() => fileInputEl?.click()} class="text-blue-400 underline hover:text-blue-300">
								עיין בקבצים
							</button>
						</p>
						<p class="text-sm text-gray-500">פורמטים נתמכים: TXT, MD, PDF, DOCX, Excel, HTML, RTF</p>
						<input
						...
						<div class="mt-6 space-y-2">
							<div class="mb-3 flex items-center justify-between">
								...
								{#if files.some((f) => f.status === "error")}
									<button onclick={() => (files = files.filter((f) => f.status !== "error"))} class="text-xs text-gray-500 hover:text-gray-300">
										נקה שגיאות
									</button>
								{/if}
							</div>
								...
								<div class="rounded-lg border border-gray-700 bg-gray-800 p-4">
									<div class="flex items-start justify-between">
											...
											<div class="flex-1">
												...
												<p class="mt-1 text-xs text-gray-500">
													...
													{#if file.status === "processing" && file.totalChunks}
														<span class="mr-2 text-blue-400">({Math.round(file.progress)}% - {file.totalChunks} חלקים)</span>
													{/if}
												</p>
												...
												{#if file.status === "completed"}
													<div class="mt-2 flex gap-4">
														<span class="text-xs text-green-400">✓ עובד בהצלחה - עובר לספריה...</span>
													</div>
												{/if}
											</div>
										</div>
										...
										{#if file.status === "pending" || file.status === "error"}
											<button onclick={() => removeFile(file.id)} class="rounded p-1 transition-colors hover:bg-gray-700" title="הסר" aria-label="הסר קובץ">
												<svg class="size-4 text-gray-500 transition-colors hover:text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
				...
				{:else}
					...
					<div>
						...
						{:else}
							<div class="space-y-2">
								<h3 class="mb-3 text-sm font-medium text-gray-400">ספריה ({existingBooks.length} ספרים)</h3>
											...
											<div class="flex-1">
												...
												<div class="flex gap-4 text-xs text-gray-400">
													...
													{#if book.upload_timestamp}
														<span class="text-gray-500">{new Date(book.upload_timestamp).toLocaleDateString("he-IL")}</span>
													{/if}
												</div>
											</div>
											<button onclick={() => (deleteConfirm = { bookId: book.book_id, bookTitle: book.title })} class="rounded p-2 transition-colors hover:bg-gray-700" title="מחק מהספריה" aria-label="מחק מהספריה">
												<svg class="size-4 text-gray-500 transition-colors hover:text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
													<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
												</svg>
											</button>
			...
			<div class="border-t border-gray-700 p-6">
				<div class="flex items-center justify-between">
					...
					<div class="flex gap-3">
						<button onclick={handleClose} class="px-4 py-2 text-gray-300 transition-colors hover:text-gray-100">
							סגור
						</button>
						{#if !showExisting}
							<button
								...
								class={["rounded-lg px-6 py-2 font-medium transition-all", files.some((f) => f.status === "pending") && !isProcessing ? "bg-blue-500 text-white hover:bg-blue-600" : "cursor-not-allowed bg-gray-700 text-gray-500"]}
							>
								{#if isProcessing}
			...
			<div class="fixed inset-0 z-[60] flex items-center justify-center bg-black/50">
				<div class="mx-4 w-full max-w-md rounded-xl border border-gray-700 bg-gray-800 p-6">
					<div class="mb-4 flex items-start gap-3">
						...
						<div class="flex-1">
							...
							<p class="text-sm text-gray-400">
								האם אתה בטוח שברצונך למחוק את <span class="font-medium text-gray-200">"{deleteConfirm.bookTitle}"</span>?
							</p>
							<p class="mt-2 text-sm text-gray-500">פעולה זו תמחק לצמיתות את כל הנתונים המעובדים כולל חלקים והטמעות.</p>
						</div>
					</div>
					{#if deleteError}
					...
					<div class="flex justify-end gap-3">
						<button onclick={() => { deleteConfirm = null; deleteError = null; }} disabled={isDeleting} class="px-4 py-2 text-gray-300 transition-colors hover:text-gray-100 disabled:opacity-50">
							ביטול
						</button>
						<button onclick={deleteBook} disabled={isDeleting} class={["flex items-center gap-2 rounded-lg px-4 py-2 transition-colors", isDeleting ? "cursor-not-allowed bg-red-500/50 text-white/70" : "bg-red-500 text-white hover:bg-red-600"]}>
							{#if isDeleting}
								<span class="size-4 animate-spin rounded-full border-2 border-white border-t-transparent"></span>
								מוחק...
							{:else}
