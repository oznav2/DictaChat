...
<div class="space-y-4">
	...
	<div>
		...
		>
			שם השרת <span class="text-red-500">*</span>
		</label>
		<input
			id="server-name"
			type="text"
			bind:value={name}
			placeholder="שם השרת שלך באנגלית בלבד למשל: My MCP Server"
	...
	<div>
		<label for="server-url" class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">
			כתובת השרת URL <span class="text-red-500">*</span>
		</label>
		<input
			id="server-url"
			type="url"
	...
	<details class="rounded-lg border border-gray-200 dark:border-gray-700">
		...
		<div class="space-y-2 border-t border-gray-200 p-4 dark:border-gray-700">
				...
				{#each headers as header, i}
					<div class="flex gap-2">
						<input
							...
							placeholder="Header שם (למשל, Authorization)"
						...
						<div class="relative flex-1">
							...
							{#if isSensitiveHeader(header.key)}
								<button
									...
									title={showHeaderValues[i] ? "הסתר ערך" : "הצג ערך"}
								>
			...
			<p class="text-xs text-gray-500 dark:text-gray-400">
				...
				><br />
				• API מפתח:
				<code class="rounded bg-gray-100 px-1 dark:bg-gray-700">X-API-Key: המפתח-שלך</code>
			</p>
		</div>
	</details>
	...
	>
		<div class="flex items-start gap-3">
			...
			<div class="text-sm leading-5">
				<p class="font-medium">עליכם להיות זהירים בהוספת שרתי MCP</p>
				<p class="mt-1 text-[13px] text-amber-800 dark:text-yellow-100/90">
					שרתי כלים מאפשרים למודל להשתמש בפונקציות וכלים שעלולים לחשוף את המידע שלך ואת תוכן השיחות שאתה מנהל בממשק
					למקומות שעלולים לעשות בו שימוש זדוני. עליך להוסיף שרתים ממקור מהימן בלבד
					מומלץ ככלל אצבע לא לשתף את המודל במידע אישי או רגיש. פרטיותך בידיך
				</p>
			</div>
		</div>
	</div>
