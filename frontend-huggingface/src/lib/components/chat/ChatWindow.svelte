<script lang="ts">
	...
	let isRTL = $derived(detectRTLLanguage(draft));
</script>
		...
		<div class="w-full">
			...
			>
				...
				{:else}
					...
					>
						...
						{:else}
							<ChatInput
								placeholder={isReadOnly ? "This conversation is read-only." : "הקלידו כאן כדי לשלוח את ההודעה"}
								{loading}
								bind:value={draft}
								bind:files
			...
			>
				...
				{#if !messages.length && !loading}
					<span>מודל ניסיוני. יכול לטעות - אנא אמתו תשובות. אל תשתפו מידע רגיש</span>
				{/if}
			</div>
		</div>
	</div>
