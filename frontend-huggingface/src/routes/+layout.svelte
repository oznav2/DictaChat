<script lang="ts">
	...
	function handleSidebarResize(width: number) {
		...
		if (!browser) return;
		window.localStorage.setItem("sidebarWidth", String(clamped));
	}
...
>
	<ExpandNavigation
		...
		offset={!isNavCollapsed ? Math.min(500, Math.max(0, sidebarWidth)) : 0}
	/>
	...
	<nav
		class="grid h-full max-h-dvh grid-cols-1 grid-rows-[auto,1fr,auto] overflow-hidden max-md:hidden"
	>
		<NavMenu
			{conversations}
			user={data.user}
