...
export const GET: RequestHandler = async () => {
	try {
		...
		const getDiagnostics = (client as unknown as { getDiagnostics?: () => EmbeddingServiceDiagnostics })?.getDiagnostics;
		...
		const isOperational = (client as unknown as { isOperational?: () => boolean })?.isOperational?.() ?? !status.isOpen;
		const isDegradedMode = (client as unknown as { isDegradedMode?: () => boolean })?.isDegradedMode?.() ?? false;
		...
		return json({
			...
			recommendations: diagnostics?.recommendations ?? (status.isOpen 
				? [
...
export const POST: RequestHandler = async ({ request }) => {
	...
	if (!["reset", "degraded"].includes(action)) {
		return json({
			success: false,
			error: "Invalid action. Supported actions: 'reset' (reset circuit breaker), 'degraded' (manage degraded mode)",
			usage: {
				reset: "POST {\"action\":\"reset\"} - Reset circuit breaker if service is healthy",
				degraded: "POST {\"action\":\"degraded\",\"enabled\":true|false} - Enable/disable graceful degradation mode",
			}
		}, { status: 400 });
	}
	...
	try {
		...
		if (action === "degraded") {
			...
			if (typeof enabled !== "boolean") {
				return json({
					success: false,
					error: "Missing or invalid 'enabled' parameter. Use {\"action\":\"degraded\",\"enabled\":true|false}",
				}, { status: 400 });
			}
			...
			return json({
				success: true,
				message: enabled ? "Degraded mode enabled - using fallback embeddings" : "Degraded mode disabled",
		...
		if (!isHealthy) {
			...
			if (diagnostics?.lastErrorCategory === "configuration") {
				recoverySteps.push("3. CONFIGURATION ERROR: Check MODEL_IDLE_TIMEOUT environment variable (must be > 0)");
				recoverySteps.push("4. Fix the configuration and restart: docker-compose restart dicta-retrieval");
			} else if (diagnostics?.lastErrorCategory === "service_down") {
				recoverySteps.push("3. SERVICE DOWN: Start the container: docker-compose up -d dicta-retrieval");
			} else {
			...
			recoverySteps.push("");
			recoverySteps.push("ALTERNATIVE: Enable graceful degradation to continue without embeddings:");
