...
<script lang="ts">
	...
	function getDeleteItemCount(collection: DeleteTarget): number {
		...
		switch (collection) {
			case "memory_bank": return dataStats.memory_bank.count;
			case "working": return dataStats.working.count;
			case "history": return dataStats.history.count;
			case "patterns": return dataStats.patterns.count;
			case "books": return dataStats.books.count;
			case "sessions": return dataStats.sessions.count;
			case "knowledge-graph": return dataStats.knowledge_graph.nodes + dataStats.knowledge_graph.edges;
			default: return 0;
		}
	}
	...
	function getDeleteMessage(collection: DeleteTarget): string {
		switch (collection) {
			case "memory_bank": return "זה ימחק לצמיתות את כל הזיכרונות שה-AI שמר עליך. כולל זהות, העדפות, מטרות והקשר.";
			case "working": return "זה ימחק את הקשר השיחה הנוכחי מ-24 השעות האחרונות.";
			case "history": return "זה ימחק היסטוריית שיחות קודמות (שמירה של 30 יום).";
			case "patterns": return "זה יסיר את כל דפוסי הפתרון המוכחים שה-AI למד.";
			case "books": return "זה ימחק את כל הספרים והמסמכים שהועלו. כולל וקטורים וגרף ידע.";
			case "sessions": return "זה ימחק את כל קבצי השיחות.";
			case "knowledge-graph": return "זה ינקה את כל יחסי המושגים והחיבורים שה-AI בנה.";
			default: return "פעולה זו לא ניתנת לביטול.";
		}
	}
	...
	const deleteItems: Array<{ key: DeleteTarget; label: string; labelEn: string; desc: string }> = [
		{ key: "memory_bank", label: "בנק זיכרון", labelEn: "Memory Bank", desc: "עובדות והעדפות משתמש" },
		{ key: "working", label: "זיכרון עובד", labelEn: "Working Memory", desc: "הקשר נוכחי (24 שעות)" },
	...
	function getTotalCount(): number {
		...
		return dataStats.memory_bank.count + dataStats.working.count + dataStats.history.count + 
			dataStats.patterns.count + dataStats.books.count + dataStats.sessions.count +
			dataStats.knowledge_graph.nodes + dataStats.knowledge_graph.edges;
	}
</script>
...
<div class="mx-auto w-full max-w-3xl p-4" dir="rtl">
	...
	<!-- Success feedback -->
	{#if lastAction}
		<div class="mt-4 flex items-center gap-3 rounded-lg border border-green-200 bg-green-50 p-3 dark:border-green-900/30 dark:bg-green-900/10">
			<CarbonCheckmarkFilled class="h-5 w-5 text-green-500" />
			<div class="text-sm text-green-700 dark:text-green-400">
				{lastAction.type}: {lastAction.count} {typeof lastAction.count === 'number' && lastAction.type !== "דחיסת מסד נתונים" ? "פריטים נמחקו" : "MB שוחזרו"}
			</div>
			<button
				type="button"
	...
	<div class="mt-4 grid gap-4">
		...
		<div class="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-800">
			...
			<!-- Warning -->
			<div class="mt-3 flex items-start gap-3 rounded-lg border border-red-200 bg-red-50 p-3 dark:border-red-900/30 dark:bg-red-900/10">
			...
			{:else}
				...
				{#if dataStats}
					<div class="mt-4 rounded-lg bg-gray-50 p-3 dark:bg-gray-900/50">
						...
						<div class="text-xl font-bold text-gray-900 dark:text-gray-100">{getTotalCount().toLocaleString()}</div>
					</div>
				{/if}
					...
					{#each deleteItems as item}
						...
						<div class="flex items-center justify-between rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-gray-700 dark:bg-gray-800/50">
							...
							<div class="flex items-center gap-3">
								<span class="min-w-[60px] text-left text-sm tabular-nums text-gray-500 dark:text-gray-400">{count} פריטים</span>
								<button
									type="button"
